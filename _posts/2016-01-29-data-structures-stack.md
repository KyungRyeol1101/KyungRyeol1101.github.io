...
- 스택을 구현하는 방법은 배열과 연결 리스트가 있다.\\배열의 큰 단점은 처음 생성한 크기를 바꿀 수 없다는 점이다.\\그래서 순차적으로 데이터를 추가하고 삭제하는 스택은 배열리스트(Array List)와 같은 배열기반의 컬렉션 클래스가 적합하다.\\overflow 스택의 모든 기억장소가 꽉 채워져 있어서 더 이상 데이터를 삽입(PUSH할 때)할 수 없다.\\underflow 자료가 없다면(TOP포인터가 주소 0을 가지고 있다면) 스택에는 삭제(POP할 때)할 자료가 없다.
